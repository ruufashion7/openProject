...
@Component({
  selector: 'app-payment-dates-v2',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './payment-dates-v2.component.html',
  styleUrl: './payment-dates-v2.component.css'
})
export class PaymentDatesV2Component implements OnInit, OnDestroy {
  // Status
    ...
    this.api.getPaymentDates()
      ...
      .subscribe({
        next: (cards) => {
          ...
          this.updateFilteredCards();
          this.status = 'idle';
          this.message = cards.length ? '' : 'No payment data available.';
        },
        error: (err: HttpErrorResponse) => {
          if (err.status === 401) {
            this.status = 'failed';
  ...
  updateTotals(): void {
    ...
    this.totalCustomers = this.filteredCards.length;
  }
  ...
  updateSearchSuggestions(): void {
    ...
    this.searchSuggestions = this.cards
      .filter(card => {
        const name = (card.customer || '').toLowerCase();
        const phone = (card.phoneNumber || '').toLowerCase();
        return name.includes(query) || phone.includes(query);
      })
      .slice(0, 5)
      .map(card => card.customer || '');
